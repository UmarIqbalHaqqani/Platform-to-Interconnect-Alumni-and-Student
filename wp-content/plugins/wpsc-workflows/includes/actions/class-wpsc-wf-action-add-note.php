...
if ( ! class_exists( 'WPSC_WF_Action_Add_Note' ) ) :

	final class WPSC_WF_Action_Add_Note {
				...
				<div class="wf-action-body">
					...
					<div class="wpsc-input-group">
						<div class="label-container">
							<label for="">
								<?php echo esc_attr( wpsc__( 'Note', 'wpsc-workflows' ) ); ?>
		...
		public static function sanitize_action( $action ) {
			...
			if ( ! $body ) {
				wp_send_json_error( esc_attr__( 'Note text not set!', 'wpsc-workflows' ), 400 );
			}
		...
		public static function execute( $action, $ticket, $workflow ) {
			...
			// avoid looping.
			$last_note = $ticket->get_last_note();
			if ( $last_note && $last_note->body == $action['body'] ) {
				return;
			}
