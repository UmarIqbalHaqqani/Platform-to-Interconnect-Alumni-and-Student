...
if ( ! class_exists( 'WPSC_WF_Action_Change_Agentonly_Fields' ) ) :

	final class WPSC_WF_Action_Change_Agentonly_Fields {
		 ...
		 * @var string
		 */
		public static $slug = 'change-agentonly-fields';
		...
		public static function print( $action = array() ) {
			...
			<div class="wf-action-item" data-slug="<?php echo esc_attr( self::$slug ); ?>">
				...
				<div class="wf-action-body">
					<div class="wf-cf-container agentonly">
						<?php
						if ( $action ) {
					...
					<button class="wpsc-button small secondary" onclick="wpsc_wf_cf_get_add_new_field( '<?php echo esc_attr( wp_create_nonce( 'wpsc_wf_cf_get_add_new_field' ) ); ?>', 'agentonly' );">
		...
		public static function execute( $action, $ticket, $workflow ) {
			...
			if ( $prev != $ticket ) {
				...
				do_action( 'wpsc_change_agentonly_fields', $prev, $ticket, 0 );
