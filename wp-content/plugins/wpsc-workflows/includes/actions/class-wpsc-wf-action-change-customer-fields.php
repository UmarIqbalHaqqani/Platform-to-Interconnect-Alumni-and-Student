...
if ( ! class_exists( 'WPSC_WF_Action_Change_Customer_Fields' ) ) :

	final class WPSC_WF_Action_Change_Customer_Fields {
		 ...
		 * @var string
		 */
		public static $slug = 'change-customer-fields';
		...
		public static function print( $action = array() ) {
			...
			<div class="wf-action-item" data-slug="<?php echo esc_attr( self::$slug ); ?>">
				...
				<div class="wf-action-body">
					<div class="wf-cf-container customer">
						<?php
						if ( $action ) {
					...
					<button class="wpsc-button small secondary" onclick="wpsc_wf_cf_get_add_new_field( '<?php echo esc_attr( wp_create_nonce( 'wpsc_wf_cf_get_add_new_field' ) ); ?>', 'customer' );">
		...
		public static function execute( $action, $ticket, $workflow ) {
			...
			foreach ( $action as $slug => $val ) {
				...
				$cf_relationship[ $cf->type::$slug ]::modify_customer( $cf, $val, $ticket );
			}
		}
	}
