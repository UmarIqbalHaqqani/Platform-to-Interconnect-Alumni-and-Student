...
if ( ! class_exists( 'WPSC_WF_Settings_Manual' ) ) :

	final class WPSC_WF_Settings_Manual {
		...
		public static function init() {
			...
			// setting actions.
			add_action( 'wp_ajax_wpsc_wf_get_manual_settings', array( __CLASS__, 'load_list' ) );

			// add new.
			add_action( 'wp_ajax_wpsc_wfm_get_add_workflow', array( __CLASS__, 'get_add_workflow' ) );
			add_action( 'wp_ajax_wpsc_wfm_set_add_workflow', array( __CLASS__, 'set_add_workflow' ) );

			// Enable/Disable.
			add_action( 'wp_ajax_wpsc_wfm_disable_workflow', array( __CLASS__, 'disable_workflow' ) );
			add_action( 'wp_ajax_wpsc_wfm_enable_workflow', array( __CLASS__, 'enable_workflow' ) );

			// edit.
			add_action( 'wp_ajax_wpsc_wfm_get_edit_workflow', array( __CLASS__, 'get_edit_workflow' ) );
			add_action( 'wp_ajax_wpsc_wfm_set_edit_workflow', array( __CLASS__, 'set_edit_workflow' ) );

			// clone.
			add_action( 'wp_ajax_wpsc_wfm_get_clone_workflow', array( __CLASS__, 'get_clone_workflow' ) );
			add_action( 'wp_ajax_wpsc_wfm_set_clone_workflow', array( __CLASS__, 'set_clone_workflow' ) );

			// Delete.
			add_action( 'wp_ajax_wpsc_wfm_delete_workflow', array( __CLASS__, 'delete_workflow' ) );
		}
					...
					foreach ( $workflows as $id => $workflow ) {
						...
						<tr>
							...
							<td>
								...
								if ( $workflow['status'] ) {
									?>
									<a href="javascript:wpsc_wfm_disable_workflow('<?php echo esc_attr( $id ); ?>', '<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_disable_workflow' ) ); ?>');" class="wpsc-link"><?php echo esc_attr( wpsc__( 'Disable', 'supportcandy' ) ); ?></a> |
									<?php
								} else {
									?>
									<a href="javascript:wpsc_wfm_enable_workflow('<?php echo esc_attr( $id ); ?>', '<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_enable_workflow' ) ); ?>');" class="wpsc-link"><?php echo esc_attr( wpsc__( 'Enable', 'supportcandy' ) ); ?></a> |
									<?php
								}
								?>
								<a href="javascript:wpsc_wfm_get_edit_workflow('<?php echo esc_attr( $id ); ?>', '<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_get_edit_workflow' ) ); ?>');" class="wpsc-link"><?php echo esc_attr( wpsc__( 'Edit', 'supportcandy' ) ); ?></a> |
								<a href="javascript:wpsc_wfm_get_clone_workflow('<?php echo esc_attr( $id ); ?>', '<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_get_clone_workflow' ) ); ?>');" class="wpsc-link"><?php echo esc_attr( wpsc__( 'Clone', 'supportcandy' ) ); ?></a> |
								<a href="javascript:wpsc_wfm_delete_workflow('<?php echo esc_attr( $id ); ?>', '<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_delete_workflow' ) ); ?>');" class="wpsc-link"><?php echo esc_attr( wpsc__( 'Delete', 'supportcandy' ) ); ?></a>
							</td>
						</tr>
						<?php
					}
					...
					buttons: [
						{
							...
							action: function ( e, dt, node, config ) {
								...
								wpsc_show_modal();
								var data = { action: 'wpsc_wfm_get_add_workflow', _ajax_nonce: '<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_get_add_workflow' ) ); ?>' };
								jQuery.post(
		...
		public static function get_add_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_get_add_workflow', '_ajax_nonce', false ) != 1 ) {
			...
			<form action="#" onsubmit="return false;" class="wpsc-wf-settings">
				...
				<input type="hidden" name="action" value="wpsc_wfm_set_add_workflow">
				<input type="hidden" name="_ajax_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_set_add_workflow' ) ); ?>">
			</form>
			<?php
			$body = ob_get_clean();
			...
			ob_start();
			?>
			<button class="wpsc-button small primary" onclick="wpsc_wfm_set_add_workflow(this);">
		...
		public static function set_add_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_set_add_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
			...
			update_option( 'wpsc-wf-manual', $workflows );
			wp_send_json(
				array(
					'index' => $index,
					'nonce' => wp_create_nonce( 'wpsc_wfm_get_edit_workflow' ),
				),
				200
			);
		}
		...
		public static function disable_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_disable_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
		...
		public static function enable_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_enable_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
		...
		public static function get_edit_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_get_edit_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
			...
			<form action="#" onsubmit="return false;" class="wpsc-wf-settings">
				...
				<input type="hidden" name="action" value="wpsc_wfm_set_edit_workflow">
				<input type="hidden" name="_ajax_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_set_edit_workflow' ) ); ?>">
		...
		public static function set_edit_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_set_edit_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
		...
		public static function get_clone_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_get_clone_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
			...
			<form action="#" onsubmit="return false;" class="wpsc-wf-settings">
				...
				<input type="hidden" name="id" value="<?php echo esc_attr( $id ); ?>">
				<input type="hidden" name="action" value="wpsc_wfm_set_clone_workflow">
				<input type="hidden" name="_ajax_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wpsc_wfm_set_clone_workflow' ) ); ?>">
			</form>
			<?php
			$body = ob_get_clean();
			...
			ob_start();
			?>
			<button class="wpsc-button small primary" onclick="wpsc_wfm_set_clone_workflow(this);">
				<?php echo esc_attr( wpsc__( 'Submit', 'supportcandy' ) ); ?>
			</button>
			<button class="wpsc-button small secondary" onclick="wpsc_close_modal();">
				<?php echo esc_attr( wpsc__( 'Cancel', 'supportcandy' ) ); ?>
		...
		public static function set_clone_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_set_clone_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
		...
		public static function delete_workflow() {
			...
			if ( check_ajax_referer( 'wpsc_wfm_delete_workflow', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorized request!', 400 );
			}
