...
if ( ! class_exists( 'WPSC_WF_CF_Checkbox' ) ) :

	final class WPSC_WF_CF_Checkbox {
		 ...
		 * @return void
		 */
		public static function print( $cf, $val = array() ) {
			...
			<div class="wpsc-input-group <?php echo esc_attr( $cf->field ); ?>" data-slug="<?php echo esc_attr( $cf->slug ); ?>">
				...
				foreach ( $options as $option ) {
					...
					<div class="checkbox-container" style="margin-bottom: 5px;">
						...
						<input id="<?php echo esc_attr( $unique_id ); ?>" <?php echo esc_attr( $checked ); ?> type="checkbox" name="actions[<?php echo esc_attr( WPSC_WF_Custom_Fields::get_action_slug( $cf->field ) ); ?>][<?php echo esc_attr( $cf->slug ); ?>][]" value="<?php echo esc_attr( $option->id ); ?>" />
		...
		public static function modify_customer( $cf, $val, $ticket ) {
			...
			$prev_val = array_map( fn( $option)=>$option->id, $customer->{$cf->slug} );

			if ( $prev_val == $val ) {
				return;
			}

			$prev_val = $prev_val ? implode( '|', $prev_val ) : '';
