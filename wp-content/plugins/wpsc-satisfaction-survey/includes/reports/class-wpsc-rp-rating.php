...
if ( ! class_exists( 'WPSC_RP_Rating' ) ) :

	final class WPSC_RP_Rating {
		...
		public static function init() {
			...
			add_action( 'allowed_cft_reports', array( __CLASS__, 'allowed_cft_report' ) );
			add_filter( 'wpsc_reports_sections', array( __CLASS__, 'add_section' ) );
			add_action( 'wp_ajax_wpsc_rp_get_rating_report', array( __CLASS__, 'layout' ) );
			add_action( 'wp_ajax_wpsc_rp_run_rating_report', array( __CLASS__, 'run_rating_reports' ) );
		}
		...
		public static function add_section( $sections ) {
			...
			foreach ( $settings['cf-reports'] as $id ) {
				...
				if ( $cf->id && class_exists( $cf->type ) && $cf->type::$slug == 'df_sf_rating' ) {
					$sections[ $cf->slug ] = array(
						'slug'     => $cf->slug,
						'icon'     => 'star',
						'label'    => $cf->name,
						'callback' => 'wpsc_rp_get_sf_rating',
						'run'      => array(
							'shortname' => 'sf_rating',
							'function'  => 'wpsc_rp_run_rating_report',
						),
					);
				}
			}
		...
		/**
		 * Print rating custom field report layout
		 *
		 * @return void
		 */
		 ...
		 * @return void
		 */
		public static function run_rating_reports() {
