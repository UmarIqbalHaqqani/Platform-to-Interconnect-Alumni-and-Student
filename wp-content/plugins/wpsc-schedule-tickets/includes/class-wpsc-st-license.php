...
if ( ! class_exists( 'WPSC_ST_License' ) ) :

	final class WPSC_ST_License {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_st_license_activate', array( __CLASS__, 'activate' ) );
			add_action( 'wp_ajax_wpsc_st_license_deactivate', array( __CLASS__, 'deactivate' ) );
		...
		public static function print_license_body() {
			...
			<div class="license-container">
				...
				if ( $license_key ) {
					?>
					<button class="wpsc-button small primary" onclick="wpsc_st_license_deactivate(this, '<?php echo esc_attr( wp_create_nonce( 'wpsc_st_license_deactivate' ) ); ?>');"><?php echo esc_attr( wpsc__( 'Deactivate' ) ); ?></button>
					<?php
					...
					?>
					<button class="wpsc-button small primary" onclick="wpsc_st_license_activate(this, '<?php echo esc_attr( wp_create_nonce( 'wpsc_st_license_activate' ) ); ?>');"><?php echo esc_attr( wpsc__( 'Activate' ) ); ?></button>
		...
		public static function activate() {
			...
			if ( check_ajax_referer( 'wpsc_st_license_activate', '_ajax_nonce', false ) != 1 ) {
		...
		public static function deactivate() {
			...
			if ( check_ajax_referer( 'wpsc_st_license_deactivate', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorised request!', 401 );
			}
			...
			unset( $licenses['schedule-tickets'] );
			update_option( 'wpsc-licenses', $licenses );
				...
				if ( ! ( is_wp_error( $response ) || $response['response']['code'] !== 200 ) ) {
					...
					if ( isset( $license_data['success'] ) && $license_data['success'] ) {
						if ( $license_data['expires'] != $license['expiry'] ) {
							...
							$licenses['schedule-tickets'] = $license;
							update_option( 'wpsc-licenses', $licenses );
						}
					}
				}
		...
		public static function load_scripts() {
			...
			<script>
				 ...
				 */
				function wpsc_st_license_activate(el, nonce) {
					...
					const data = { 
						action: 'wpsc_st_license_activate',
				 ...
				 * Deactivate license
				 */
				function wpsc_st_license_deactivate(el, nonce) {
					...
					jQuery(el).text(supportcandy.translations.please_wait);
					const data = { 
						action: 'wpsc_st_license_deactivate',
						license_key: licenseKey,
						_ajax_nonce: nonce
					};
					jQuery.post(supportcandy.ajax_url, data, function (response) {
