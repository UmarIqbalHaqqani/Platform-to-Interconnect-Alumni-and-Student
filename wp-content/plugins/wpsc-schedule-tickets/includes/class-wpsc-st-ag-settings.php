...
if ( ! class_exists( 'WPSC_ST_AG_Settings' ) ) :

	final class WPSC_ST_AG_Settings {
		...
		public static function init() {
			...
			// scheduled ticket agent role menu.
			...
			// schedule action in an individual ticket.
			add_filter( 'wpsc_individual_ticket_actions', array( __CLASS__, 'it_add_action' ), 10, 2 );

			// add scheule ticket from an individual ticket.
			add_action( 'wp_ajax_wpsc_it_get_schedule_ticket', array( __CLASS__, 'it_get_schedule_ticket' ) );
			add_action( 'wp_ajax_nopriv_wpsc_it_get_schedule_ticket', array( __CLASS__, 'it_get_schedule_ticket' ) );
			add_action( 'wp_ajax_wpsc_it_set_schedule_ticket', array( __CLASS__, 'it_set_schedule_ticket' ) );
			add_action( 'wp_ajax_nopriv_wpsc_it_set_schedule_ticket', array( __CLASS__, 'it_set_schedule_ticket' ) );

			// schedule ticket/thread source.
		...
		public static function add_ticket_source( $sources ) {
			...
			$sources['st'] = esc_attr__( 'Schedule Ticket', 'wpsc-st' );
			return $sources;
		}
		...
		public static function add_agent_role_other_permissions() {
			?>
			<div>
				<input name="caps[]" type="checkbox" value="modify-schedule-ticket">
				<span><?php esc_attr_e( 'Modify scheduled ticket', 'wpsc-st' ); ?></span>
			</div>
			<?php
		}
		 ...
		 *
		 * @param array  $args - arguments name array.
		 * @param string $caps - capability name.
		 * @return array
		 */
		public static function set_add_agent_role_other_permission( $args, $caps ) {

			$args['caps']['modify-schedule-ticket'] = in_array( 'modify-schedule-ticket', $caps ) ? true : false;
			return $args;
		}
		...
		public static function edit_agent_role_other_permissions( $role ) {
			...
			$checked = $role['caps']['modify-schedule-ticket'] ? 'checked' : ''
			?>
			<div>
				<input name="caps[]" type="checkbox" <?php echo esc_attr( $checked ); ?> value="modify-schedule-ticket">
				<span><?php esc_attr_e( 'Modify scheduled ticket', 'wpsc-st' ); ?></span>
			</div>
			<?php
		}
		...
		public static function set_edit_agent_role_other_permission( $new, $prev, $caps ) {
			...
			$new['caps']['modify-schedule-ticket'] = in_array( 'modify-schedule-ticket', $caps ) ? true : false;
			return $new;
		}

		/**
		 * Add schedule action in an individual ticket
		 *
		 * @param array       $actions - action.
		 * @param WPSC_Ticket $ticket - ticket object.
		 * @return array
		 */
		public static function it_add_action( $actions, $ticket ) {

			$current_user      = WPSC_Current_User::$current_user;
			$schedule_settigns = get_option( 'wpsc-st-general-settings' );

			if (
				$schedule_settigns['schedule-existing'] &&
				WPSC_Individual_Ticket::$view_profile == 'agent' &&
				$current_user->agent->has_cap( 'modify-schedule-ticket' )
			) {

				$actions['schedule'] = array(
					'label'    => esc_attr__( 'Schedule', 'wpsc-st' ),
					'callback' => 'wpsc_it_get_schedule_ticket(' . $ticket->id . ', \'' . wp_create_nonce( 'wpsc_it_get_schedule_ticket' ) . '\');',
				);
			}
		...
		/**
		 * Schedule ticket popup
		 *
		 * @return void
		 */
		public static function it_get_schedule_ticket() {

			if ( check_ajax_referer( 'wpsc_it_get_schedule_ticket', '_ajax_nonce', false ) != 1 ) {
			...
			$title  = esc_attr__( 'Schedule Ticket', 'wpsc-st' );
			...
			ob_start();
			?>
			<form action="#" onsubmit="return false;" class="wpsc-it-schedule">
				...
				<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12 recurrence weekly">
					...
					<div class="wpsc-st-rec-setting-radio-container">
						<?php $no_of_weeks = '<input type="number" name="weekly-x-weeks" value="' . $rule['weekly-x-weeks'] . '">'; ?>
				...
				<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12 recurrence monthly">
					...
					<div class="wpsc-st-rec-setting-radio-container">
						...
						<select name="monthly-week-number-day">
							...
							for ( $i = 1; $i <= 7; $i++ ) {
								?>
								<option <?php selected( $rule['monthly-week-number-day'], $i ); ?> value="<?php echo esc_attr( $i ); ?>"><?php echo esc_attr( WPSC_Functions::get_day_name( $i ) ); ?></option>
								<?php
							}
							?>
						</select>

						<?php $day = ob_get_clean(); ?>
				...
				<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12 recurrence yearly">
					...
					<div class="wpsc-st-rec-setting-radio-container">
						...
						<?php $month = ob_get_clean(); ?>
						<input type="radio" <?php echo $rule['yearly-recurrence-type'] == 'yearly-day-number' ? 'checked' : ''; ?> name="yearly-recurrence-type" value="yearly-day-number" checked>
					...
					<div class="wpsc-st-rec-setting-radio-container">
						...
						<?php $month = ob_get_clean(); ?>
						<input type="radio" <?php echo $rule['yearly-recurrence-type'] == 'yearly-week-number' ? 'checked' : ''; ?> name="yearly-recurrence-type" value="yearly-week-number">
				...
				<input type="hidden" name="action" value="wpsc_it_set_schedule_ticket">
				<input type="hidden" name="ticket_id" value="<?php echo esc_attr( $ticket->id ); ?>">
				<input type="hidden" name="_ajax_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wpsc_it_set_schedule_ticket' ) ); ?>">
			</form>
			<?php
			$body = ob_get_clean();
			...
			ob_start();
			?>
			<button class="wpsc-button small primary" onclick="wpsc_it_set_schedule_ticket(this);">
		...
		/**
		 * Set schedule ticket in an individual ticket
		 *
		 * @return void
		 */
		public static function it_set_schedule_ticket() {

			if ( check_ajax_referer( 'wpsc_it_set_schedule_ticket', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorised request!', 401 );
			}
			...
			if ( ! $title ) {
				wp_send_json_error( esc_attr__( 'Please insert title', 'wpsc-st' ), 400 );
			}
			$rule['title'] = $title;
			...
			// customer.
			$rule['customer'] = $ticket->customer->id;

			// subject.
			$rule['subject'] = stripslashes( $ticket->subject );

			// description.
			$description         = $ticket->get_description_thread();
			$description         = ! $description ? esc_attr__( 'No description found!', 'wpsc-st' ) : $description;
			$rule['description'] = stripslashes( $description->body );

			// additional recipients.
			$add_recipients         = $ticket->add_recipients;
			...
			foreach ( WPSC_Custom_Field::$custom_fields as $cf ) {
				...
				if (
					...
					in_array( $cf->slug, array( 'status', 'category', 'priority', 'add_recipients' ) ) ||
					in_array( $cf->type::$slug, WPSC_ST_Settings_CRUD::$ignore_cft )
				) {
					continue;
				}

				$rule[ $cf->slug ] = $cf->type::get_duplicate_ticket_data( $cf, $ticket );
			}
