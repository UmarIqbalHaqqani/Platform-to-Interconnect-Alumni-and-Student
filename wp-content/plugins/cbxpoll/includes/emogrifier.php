...
/* ------------------------------------------------------------------------
	...
	# Websites: https://codeboxr.com
	# Technical Support:- https://codeboxr.com/product/cbx-forms-for-wordpress/
	------------------------------------------------------------------------- */
...
if (!class_exists('CBXPollEmogrifier')):
    class CBXPollEmogrifier
    {
        /**
         * @var string
         */
        ...
        public function emogrify()
        {
            ...
            if ($this->preserveEncoding) {
                if (function_exists('mb_convert_encoding')) {
                    ...
                    return htmlspecialchars_decode(utf8_encode(html_entity_decode($xmlDocument->saveHTML(), ENT_COMPAT,
                        self::ENCODING)));
                }
            } else {
        ...
        {
            $css = preg_replace_callback('#@media\\s+(?:only\\s)?(?:[\\s{\(]|screen|all)\\s?[^{]+{.*}\\s*}\\s*#misU',
                array(
                    $this,
                    '_media_concat'
                ), $css);
        ...
        private function getUnifiedHtml()
        {
            if (!empty($this->unprocessableHtmlTags)) {
                ...
                $bodyWithoutUnprocessableTags = preg_replace('/<\\/?('.$unprocessableHtmlTags.')[^>]*>/i', '',
                    $this->html);
            } else {
            ...
            if (function_exists('mb_convert_encoding')) {
                ...
                return htmlspecialchars_decode(utf8_decode(htmlentities($bodyWithoutUnprocessableTags, ENT_COMPAT,
                    self::ENCODING, false)));
            }
        }
        ...
        private function translateNthChild(array $match)
        {
            ...
            if (isset($result[self::MULTIPLIER])) {
                if ($result[self::MULTIPLIER] < 0) {
                    ...
                    return sprintf('*[(last() - position()) mod %u = %u]/self::%s', $result[self::MULTIPLIER],
                        $result[self::INDEX], $match[1]);
                } else {
                    return sprintf('*[position() mod %u = %u]/self::%s', $result[self::MULTIPLIER],
                        $result[self::INDEX], $match[1]);
                }
            } else {
        ...
        private function translateNthOfType(array $match)
        {
            ...
            if (isset($result[self::MULTIPLIER])) {
                if ($result[self::MULTIPLIER] < 0) {
                    ...
                    return sprintf('%s[(last() - position()) mod %u = %u]', $match[1], $result[self::MULTIPLIER],
                        $result[self::INDEX]);
                } else {
                    return sprintf('%s[position() mod %u = %u]', $match[1], $result[self::MULTIPLIER],
                        $result[self::INDEX]);
                }
            } else {
