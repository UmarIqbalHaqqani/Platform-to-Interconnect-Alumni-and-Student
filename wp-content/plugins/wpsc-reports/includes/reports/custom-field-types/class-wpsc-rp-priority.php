...
if ( ! class_exists( 'WPSC_RP_Priority' ) ) :

	final class WPSC_RP_Priority {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_rp_get_priority', array( __CLASS__, 'layout' ) );
			add_action( 'wp_ajax_wpsc_rp_run_priority_report', array( __CLASS__, 'run_priority_reports' ) );
		}

		/**
		 * Add priority menu in reports
		 *
		...
		public static function add_section( $sections ) {
			...
			foreach ( $settings['cf-reports'] as $id ) {
				...
				if ( $cf->id && class_exists( $cf->type ) && $cf->type::$slug == 'df_priority' ) {
					$sections[ $cf->slug ] = array(
						'slug'     => $cf->slug,
						'icon'     => 'chart-bar',
						'label'    => $cf->name,
						'callback' => 'wpsc_rp_get_priority',
						'run'      => array(
							'shortname' => 'cfp',
							'function'  => 'wpsc_rp_run_priority_report',
						),
					);
				}
			}
		 ...
		 * @return void
		 */
		public static function run_priority_reports() {
