...
if ( ! class_exists( 'WPSC_RP_Filters' ) ) :
	...
	final class WPSC_RP_Filters {
			...
			<form onsubmit="return false;" class="wpsc-report-filters">
				<div class="wpsc-setting-filter-container">
					...
					<div class="setting-filter-item wpsc-filter-submit" style="flex-direction: row !important;">
						...
						<button class="wpsc-button small secondary margin-right" onclick="wpsc_print_report()"><?php echo esc_attr( wpsc__( 'Print', 'supportcandy' ) ); ?></button>
		...
		public static function get_custom_filter() {
			...
			$filter    = isset( $saved_filters[ $filter_id ] ) ? $saved_filters[ $filter_id ] : array();
			if ( $filter ) { // set filters if set.
				$filters = $filter['filters'];
				$filters = str_replace( '^^', '\n', $filters );
			}
			$filters = $filters ? json_decode( html_entity_decode( $filters ), true ) : array();
							...
							<div class="content">
								<div class="item">
									<select class="cf" onchange="wpsc_get_ticket_filter_operators(this, '<?php echo esc_attr( wp_create_nonce( 'wpsc_get_ticket_filter_operators' ) ); ?>');">
										<option value=""><?php echo esc_attr( wpsc__( 'Select field', 'supportcandy' ) ); ?></option>
										<?php
										foreach ( $custom_fields as $cfield ) :
											if ( ! $cfield->type::$is_filter || $cfield->field == 'usergroup' ) {
												continue;
											}
											if ( in_array( $cfield->slug, $ignore_filters ) ) {
												continue;
											}
						...
						<div class="item">
							<select class="cf" onchange="wpsc_get_ticket_filter_operators(this, '<?php echo esc_attr( wp_create_nonce( 'wpsc_get_ticket_filter_operators' ) ); ?>');">
								...
								foreach ( $custom_fields as $cf ) :
									if ( ! $cf->type::$is_filter || $cf->field == 'usergroup' ) {
										continue;
									}
									if ( in_array( $cf->slug, $ignore_filters ) ) {
										continue;
									}
									?>
