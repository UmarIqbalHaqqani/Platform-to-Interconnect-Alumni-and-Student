...
function wpsc_set_filter_duration_dates(duration) {
	...
	switch (duration) {
		...
		case 'this-month':
			dateStr = new Date( date.getFullYear(), date.getMonth(), 1 ).toLocaleDateString( 'en-CA' );
			jQuery( '.setting-filter-item.from-date' ).find( 'input' ).val( dateStr );
			dateStr = new Date( date.getFullYear(), date.getMonth() + 1, 0 ).toLocaleDateString( 'en-CA' );
			jQuery( '.setting-filter-item.to-date' ).find( 'input' ).val( dateStr );
			break;
		...
		case 'this-quarter':
			...
			dateStr = new Date( date.getFullYear(), (quarter - 1) * 3, 1 ).toLocaleDateString( 'en-CA' );
			jQuery( '.setting-filter-item.from-date' ).find( 'input' ).val( dateStr );
			if (quarter == 4) {
				dateStr = new Date( date.getFullYear(), 11, 31 ).toLocaleDateString( 'en-CA' );
			} else {
				dateStr = new Date( date.getFullYear(), quarter * 3, 0 ).toLocaleDateString( 'en-CA' );
			}
			jQuery( '.setting-filter-item.to-date' ).find( 'input' ).val( dateStr );
			break;
		...
		case 'last-quarter':
			quarter = Math.floor( (date.getMonth() + 3) / 3 );
			if (quarter == 1) {
				dateStr = new Date( date.getFullYear() - 1, 9, 1 ).toLocaleDateString( 'en-CA' );
				jQuery( '.setting-filter-item.from-date' ).find( 'input' ).val( dateStr );
				dateStr = new Date( date.getFullYear() - 1, 11, 31 ).toLocaleDateString( 'en-CA' );
				jQuery( '.setting-filter-item.to-date' ).find( 'input' ).val( dateStr );
			} else {
				dateStr = new Date( date.getFullYear(), (quarter - 2) * 3, 1 ).toLocaleDateString( 'en-CA' );
				jQuery( '.setting-filter-item.from-date' ).find( 'input' ).val( dateStr );
				dateStr = new Date( date.getFullYear(), (quarter - 1) * 3, 0 ).toLocaleDateString( 'en-CA' );
				jQuery( '.setting-filter-item.to-date' ).find( 'input' ).val( dateStr );
			}
			break;

		case 'this-year':
			dateStr = new Date( date.getFullYear(), 0, 1 ).toLocaleDateString( 'en-CA' );
			jQuery( '.setting-filter-item.from-date' ).find( 'input' ).val( dateStr );
			dateStr = new Date( date.getFullYear(), 11, 31 ).toLocaleDateString( 'en-CA' );
			jQuery( '.setting-filter-item.to-date' ).find( 'input' ).val( dateStr );
			break;
		...
		case 'last-year':
			dateStr = new Date( date.getFullYear() - 1, 0, 1 ).toLocaleDateString( 'en-CA' );
			jQuery( '.setting-filter-item.from-date' ).find( 'input' ).val( dateStr );
			dateStr = new Date( date.getFullYear() - 1, 11, 31 ).toLocaleDateString( 'en-CA' );
			jQuery( '.setting-filter-item.to-date' ).find( 'input' ).val( dateStr );
			break;
...
function wpsc_rp_get_baches() {
	...
	if (diffDays == 1) { // one day.
		...
		for (var i = 0; i <= 23; i++) {
			...
			batchData.push(
				{
					fromDate: fromDate.toLocaleDateString( 'en-CA' ) + ' ' + hr + ':00:00',
					toDate: fromDate.toLocaleDateString( 'en-CA' ) + ' ' + hr + ':59:59',
					durationType: 'day'
				}
			);
		}
		...
		do {
			batchData.push(
				{
					fromDate: fromDate.toLocaleDateString( 'en-CA' ) + ' 00:00:00',
					toDate: fromDate.toLocaleDateString( 'en-CA' ) + ' 23:59:59',
					durationType: 'days'
				}
			);
		...
		var from = fromDate.toLocaleDateString( 'en-CA' );
		...
		do {
			...
			batchData.push(
				{
					fromDate: from.toLocaleDateString( 'en-CA' ) + ' 00:00:00',
					toDate: to.toLocaleDateString( 'en-CA' ) + ' 23:59:59',
					durationType: 'weeks'
				}
			);
		} while (fromDate < toDate);
